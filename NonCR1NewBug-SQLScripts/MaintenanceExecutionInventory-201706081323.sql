...
/****** Object:  UserDefinedFunction [dbo].[GetDeltaViewFunction]    Script Date: 06/08/2017 13:03:00 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetDeltaViewFunction]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[GetDeltaViewFunction]
GO
...
CREATE FUNCTION [dbo].[GetDeltaViewFunction]
(	
...
--DECLARE @LocationCode VARCHAR(50) = 'ID21'
--DECLARE @InventoryDate DATETIME = '2016-10-12 00:00:00.000'
...
--Luki: Item Convert (transaction date) Status: Source=Destination 
  ...
  FROM [dbo].[MntcEquipmentItemConvert]
  group by TransactionDate,LocationCode,ItemCodeSource,SourceStatus,UpdatedDate) a,
              [MntcInventory] b
         WHERE a.TransactionDate = @InventoryDate
           AND b.InventoryDate = @InventoryDate
...
--              [MntcInventory] b
--         WHERE a.TransferDate = @InventoryDate
--           AND b.InventoryDAte = @InventoryDate
--           AND a.LocationCodeSource = @LocationCode
--           AND b.LocationCode = @LocationCode
...
--         FROM [MntcEquipmentMovement] a,
--              [MntcInventory] b
--         WHERE a.ReceiveDate = @InventoryDate
--           AND b.InventoryDate = @InventoryDate
--           AND a.LocationCodeDestination = @LocationCode
--           AND b.LocationCode = @LocationCode
...
--Wahyu: Item Movement (transaction date) source
         SELECT b.InventoryDate,
                b.LocationCode,
                b.UnitCode,
                b.ItemStatus,
         ...
         GROUP BY b.InventoryDate,
                  ...
                  b.ItemCode
         UNION ALL
--Wahyu: Item Movement (update next date) source
         SELECT b.InventoryDate,
                b.LocationCode,
                b.UnitCode,
                b.ItemStatus,
         ...
         GROUP BY b.InventoryDate,
                  ...
                  b.ItemCode
         UNION ALL
--Wahyu: Item Movement (transaction date) destination
         SELECT b.InventoryDate,
                b.LocationCode,
                b.UnitCode,
                b.ItemStatus,
         ...
         GROUP BY b.InventoryDate,
                  ...
                  b.ItemCode
         UNION ALL
--Wahyu: Item Movement (update next date) destination
         SELECT b.InventoryDate,
                b.LocationCode,
                b.UnitCode,
                b.ItemStatus,
...
----              [MntcInventory] b
----         WHERE a.AdjustmentDate = @InventoryDate
----           AND b.InventoryDate = @InventoryDate
----           AND a.LocationCode = @LocationCode
----           AND b.LocationCode = @LocationCode
----           AND a.UnitCode = b.UnitCode
...
--Luki: Adjustment (update next date) destination (add: 01/06/2016)
...
--Luki: ItemUsage (Update next date) 'BAD STOCK'
     ...
     WHERE not (x.DeltaStaw = 0 and x.DeltaStockIn = 0 and x.DeltaStockOut = 0 and x.DeltaStak = 0)   
           AND x.LocationCode = @LocationCode
...
WHERE qqq.InventoryDate = @InventoryDate
