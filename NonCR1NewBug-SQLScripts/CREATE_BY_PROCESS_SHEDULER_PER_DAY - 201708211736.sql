USE [SKTIS]
GO
/****** Object:  StoredProcedure [dbo].[CREATE_BY_PROCESS_SHEDULER_PER_DAY]    Script Date: 21-Aug-17 10:41:10 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[CREATE_BY_PROCESS_SHEDULER_PER_DAY]
AS
BEGIN
...
BEGIN TRY
	...
	BEGIN
		...
		select LocationCode, UnitCode, BrandCode, DATEADD(day,-1,max(ProductionDate)) as LastProductionDate
		...
		BEGIN
			...
			SELECT [LocationCode]
				   ...
				   ,0
				   ,0
				   ,[EndingStock] as [BeginningStock]
				   ,[EndingStock] as [EndingStock]
			...
			LocationCode = @LocationCode AND UnitCode = @UnitCode AND BrandCode = @BrandCode AND ProductionDate = DATEADD(DAY,-1,@currDate)
			AND DATEADD(DAY,-1,@currDate) < @currDate;
