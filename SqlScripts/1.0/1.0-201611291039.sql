...
BEGIN
	...
	WHILE @@FETCH_STATUS = 0 BEGIN
		...
		--select @totalTransactionlogSubmit
		...
		BEGIN
			...
			WHERE TransactionCode LIKE 'WPC/'+@LocationCodeCursor+'/1/'+@UnitCodeCursor+'/%/%/'+@YEAR+'/'+@WEEK+'/'+@DAY+'/%' ORDER BY UpdatedDate DESC);
