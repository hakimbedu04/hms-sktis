/****** Object:  StoredProcedure [dbo].[GenerateP1TemplateAP1]    Script Date: 11/16/2016 15:32:20 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GenerateP1TemplateAP1]') AND type in (N'P', N'PC'))
	...
	BEGIN
		...
		BEGIN
			...
			CASE UTL.IDFlow
				...
				WHEN 51 THEN 'END'
				ELSE 'OPEN'
			END AS FlowStatus, TFH.BrandGroupCode, TFH.CreatedDate, MTI.VendorNumber
			FROM 
	...
	BEGIN
		...
		BEGIN
			...
			CASE UTL.IDFlow
				...
				WHEN 51 THEN 'END'
				ELSE 'OPEN'
			END AS FlowStatus, TFH.BrandGroupCode, TFH.CreatedDate, '' as VendorNumber
			FROM 
				(SELECT a.TPOFeeCode, a.Calculate, a.OrderFeeType,b.LocationCode FROM [TPOFeeCalculation] a
				INNER JOIN [TPOFeeHdr] b
				ON a.TPOFeeCode=b.TPOFeeCode
