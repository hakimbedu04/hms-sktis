...
@revisiontype VARCHAR(1)
 -- autihor : Hakim
 -- desc    : add parameter revisiontype and use it in query
...
BEGIN
	...
	--SET @locationCode = 'ID26'
	--SET @unitCode = '2022'
	--SET @shift = '1'
	--SET @Date = CONVERT(DATETIME,'2017-01-04')
	--SET @brandGroupCode = 'DSS12SR-20'  --'SAO12SR-20'--
	--SET @brandCode = 'FA010783.18' --'FA044773.05' --
	--SET @RoleId = 14
	--SET @revisiontype = 1
	SET @DAY = (SELECT Case (SELECT DATEPART( DW, @Date)-1) WHEN 0 THEN 7 ELSE (SELECT DATEPART( DW, @Date)-1) END);
	SET @WEEK = (SELECT [Week] FROM [dbo].[MstGenWeek] WHERE StartDate <= @Date AND EndDate >= @Date );
	...
	BEGIN
		...
		from (
			...
			where TransactionCode like 'WPC/'+@locationCode+'/1/'+@unitCode+'/%/'+@brandCode+'/'+@YEAR+'/'+@WEEK+'/'+@DAY+'/' + @revisiontype
		...
		WHERE TransactionCode LIKE 'WPC/'+@locationCode+'/1/'+@unitCode+'/%/'+@brandCode+'/'+@YEAR+'/'+@WEEK+'/'+@DAY+'/' + @revisiontype ORDER BY UpdatedDate DESC);
		...
		--ELSE
		--BEGIN
		--	SELECT @totalEntryVerification = (SELECT COUNT(*) FROM ExePlantProductionEntryVerificationView a
		--	WHERE convert(date,a.UpdatedDate) > (select max(closingdate) from MstClosingPayroll where ClosingDate <= @Date)
		--	AND SUBSTRING(GroupCode,2,1) != '5'and ProductionEntryCode like 'EBL/'+@locationCode+'/1/'+@unitCode+'/%/'+@brandCode+'/'+@YEAR+'/'+@WEEK+'/'+@DAY
		--	GROUP BY BrandCode);
		--END
		...
		BEGIN
			SELECT @totalEntryVerification = (SELECT COUNT(*) FROM ExePlantProductionEntryVerificationView
												WHERE
												(
												convert(date,UpdatedDate) = @Date
												AND 
												(
													(convert(date,ProductionDate) >= @Date)
													AND 
													(convert(date,ProductionDate) <= @Date)
												)
											)AND 
											SUBSTRING(GroupCode,2,1) != '5' AND
											ProductionEntryCode like 'EBL/'+@locationCode+'/'+@shift+'/'+@unitCode+'/%/'+@brandCode+'/'+@YEAR+'/'+@WEEK+'/'+@DAY);
		END
		ELSE
		BEGIN
			SELECT @totalEntryVerification = (SELECT COUNT(*) FROM ExePlantProductionEntryVerificationView
			WHERE 
			(convert(date,ProductionDate) = @Date) AND 
			SUBSTRING(GroupCode,2,1) != '5' AND 
			ProductionEntryCode like 'EBL/'+@locationCode+'/'+@shift+'/'+@unitCode+'/%/'+@brandCode+'/'+@YEAR+'/'+@WEEK+'/'+@DAY);
		END
		...
		BEGIN
			...
			IF(@RoleId = @idNextRole or @RoleId=3)
			BEGIN
		...
		BEGIN
			...
			IF(@RoleId = @idNextRole or @RoleId=5)
			BEGIN
	...
	BEGIN
		...
		WHILE @@FETCH_STATUS = 0 BEGIN 
			...
			from (
					...
					,max([IDFlow]) IDFlow
					FROM [UtilTransactionLogs] a
					where TransactionCode like 'WPC/'+@locationCode+'/1/'+@unitCode+'/%/'+@BrandCodeCursor+'/'+@YEAR+'/'+@WEEK+'/'+@DAY+'/' + @revisiontype
					and a.CreatedDate=(select MAX(CreatedDate) from
					[dbo].[UtilTransactionLogs]
					where TransactionCode=a.TransactionCode)
					group by a.TransactionCode
			...
			-- SELECT CONDITIONAL
			SELECT @LastTransLogs = (SELECT TOP 1 IDFlow FROM UtilTransactionLogs
			WHERE TransactionCode LIKE 'WPC/'+@locationCode+'/1/'+@unitCode+'/%/'+@BrandCodeCursor+'/'+@YEAR+'/'+@WEEK+'/'+@DAY+'/' + @revisiontype ORDER BY UpdatedDate DESC);
			...
			BEGIN
				SELECT @totalEntryVerification = (SELECT COUNT(*) FROM ExePlantProductionEntryVerificationView
												...
												SUBSTRING(GroupCode,2,1) != '5' AND
												ProductionEntryCode like 'EBL/'+@locationCode+'/'+@shift+'/'+@unitCode+'/%/'+@BrandCodeCursor+'/'+@YEAR+'/'+@WEEK+'/'+@DAY);
			END
			ELSE
			BEGIN
				SELECT @totalEntryVerification = (SELECT COUNT(*) FROM ExePlantProductionEntryVerificationView
