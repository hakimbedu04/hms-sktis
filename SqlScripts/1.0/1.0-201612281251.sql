IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[WAGES_ABSENT_REPORT_BYGROUP]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[WAGES_ABSENT_REPORT_BYGROUP]
...
CREATE PROCEDURE [dbo].[WAGES_ABSENT_REPORT_BYGROUP]
(
		@paramStartDate DATE
	,	@paramEndDate	DATE
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SET 
				 Alpa = CASE WHEN @sktAbsentType = 'A' THEN Alpa - @countAbsent ELSE Alpa END 
				, Ijin = CASE WHEN @sktAbsentType = 'I' THEN Ijin - @countAbsent ELSE Ijin END 
