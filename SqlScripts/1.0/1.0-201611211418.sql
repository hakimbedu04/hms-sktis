	...
	BEGIN
		...
		BEGIN
			...
			CASE UTL.IDFlow
				...
				WHEN 51 THEN 'END'
				ELSE 'OPEN'
			END AS FlowStatus, TFH.BrandGroupCode, TFH.CreatedDate, MTI.VendorNumber, MGL.CostCenter
			FROM 
	...
	BEGIN
		...
		BEGIN
			...
			CASE UTL.IDFlow
				...
				WHEN 51 THEN 'END'
				ELSE 'OPEN'
			END AS FlowStatus, TFH.BrandGroupCode, TFH.CreatedDate, '' as VendorNumber,MGL.CostCenter
			FROM 
				(SELECT a.TPOFeeCode, a.Calculate, a.OrderFeeType,b.LocationCode FROM [TPOFeeCalculation] a
				INNER JOIN [TPOFeeHdr] b
				ON a.TPOFeeCode=b.TPOFeeCode
