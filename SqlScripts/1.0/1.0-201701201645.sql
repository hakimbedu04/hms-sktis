/****** Object:  StoredProcedure [dbo].[INSERT_WORKER_ASSIGNMENT]    Script Date: 1/20/2017 4:22:49 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT TOP 1 @EmployeeNumber = EmployeeNumber FROM MstPlantEmpJobsDataAcv WHERE EmployeeID = @EmployeeID

		SELECT TOP 1 @DestinationProcessGroup = ProcessGroup FROM MstGenProcess WHERE ProcessIdentifier = SUBSTRING(@DestinationGroupCode, 1, 1);
		...
		BEGIN
			...
			BEGIN /** Assignment to PLANT Destination Location **/
				...
				BEGIN
					...
					IF NOT EXISTS
					(
						SELECT * FROM ExePlantProductionEntry WHERE ProductionEntryCode = @ProductionEntryCodeDestinationDummy AND EmployeeID = @EmployeeID
					)
					BEGIN
						INSERT INTO [dbo].[ExePlantProductionEntry]
						   ([ProductionEntryCode]
