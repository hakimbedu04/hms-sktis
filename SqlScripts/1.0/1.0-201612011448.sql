/****** Object:  StoredProcedure [dbo].[ExtendedExeReportByProcess]    Script Date: 11/15/2016 15:52:26 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	(
		...
		('Pack') as Uom,
		(6) AS UOMOrder,
		(max(e.Production) / max(mgbg.StickPerPack)) as Production,
		--max(e.KeluarBersih) as KeluarBersih,
		(max(e.KeluarBersih)/max(mgbg.StickPerPack)) as KeluarBersih,
...
--Luki Edited End
	...
	(
		...
		(max(e.Production) / max(mgbg.StickPerPack)) as Production,
		--max(e.KeluarBersih) as KeluarBersih,
		(max(e.Production) / max(mgbg.StickPerPack)) - (@rejectSampleUOM7 / mgbg.StickPerPack) as KeluarBersih,
		(@rejectSampleUOM7 / mgbg.StickPerPack) as RejectSample,
	...
	(
		...
		max(UOM) as Uom,
		(9) AS UOMOrder,
		@keluarBersihUOM7 as Production,
		...
		ROUND(ISNULL(((select TOP 1 EndingStock from ExeReportByProcess xy
			where 
				...
				--and KPSYear = SOURCE.KPSYear
				--and KPSWeek = SOURCE.KPSWeek
		) + max(Production) - max(KeluarBersih) - max(RejectSample)),0), 2) as EndingStock,
	...
	(
		...
		('Slof') as Uom,
		(10) AS UOMOrder,
		@keluarBersihUOM7 / max(mgbg.StickPerSlof) as Production,
		--max(e.KeluarBersih) as KeluarBersih,
		@keluarBersihUOM7/max(mgbg.StickPerSlof) as KeluarBersih,
		0 as RejectSample,
		ROUND(ISNULL((select TOP 1 EndingStock from ExeReportByProcess xy
			where 
				xy.ProductionDate = DATEADD(day, -1, @ProductionDate) 
	...
	(
		...
		(11) AS UOMOrder,
		@keluarBersihUOM7 as Production,
		@keluarBersihUOM7 as KeluarBersih,
		0 as RejectSample,
		ROUND(ISNULL((select TOP 1 EndingStock from ExeReportByProcess xy
			where 
				xy.ProductionDate = DATEADD(day, -1, @ProductionDate) 
	...
	(
		...
		('Box') as Uom,
		(12) AS UOMOrder,
		(@keluarBersihUOM7 / max(mgbg.StickPerBox)) as Production,
		--max(e.KeluarBersih) as KeluarBersih,
		(@keluarBersihUOM7/max(mgbg.StickPerBox)) as KeluarBersih,
		0 as RejectSample,
		ROUND(ISNULL((select TOP 1 EndingStock from ExeReportByProcess xy
			where 
				xy.ProductionDate = DATEADD(day, -1, @ProductionDate) 
	...
	(
		...
		0 as Production,
		--max(e.KeluarBersih) as KeluarBersih,
		@extMoveAdj / max(mgbg.StickPerBox) as KeluarBersih,
		0 as RejectSample,
		ROund(ISNULL((select TOP 1 EndingStock from ExeReportByProcess xy
			where 
				xy.ProductionDate = DATEADD(day, -1, @ProductionDate) 
