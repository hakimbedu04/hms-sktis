IF EXISTS (SELECT * FROM   sys.objects WHERE  object_id = OBJECT_ID(N'[dbo].[GetReportByStatus]') AND type IN ( N'FN', N'IF', N'TF', N'FS', N'FT' ))
  DROP FUNCTION [dbo].[GetReportByStatus]
...
CREATE FUNCTION [dbo].[GetReportByStatus]
(	
...
BEGIN
	...
	BEGIN
		...
		WHERE g.LocationCode IN (SELECT LocationCode FROM [dbo].[GetLastChildLocation](@paramLocationCode)) AND g.ProductionDate BETWEEN @paramProdDateFrom AND @paramProdDateTo
				AND g.BrandGroupCode = @paramBrandGroup AND g.BrandCode = @paramBrandCode AND g.Shift = @paramShift
		GROUP BY
			g.ProcessGroup,
			g.StatusEmp,
			p.ProcessOrder
		...
		DECLARE @processGroup VARCHAR(16), @statusEmp VARCHAR(16), @actual INT, @prodstick INT;
		...
		BEGIN
			...
			FROM ExeReportByGroups g
			WHERE g.LocationCode IN (SELECT LocationCode FROM [dbo].[GetLastChildLocation](@paramLocationCode)) AND g.ProductionDate BETWEEN @paramProdDateFrom AND @paramProdDateTo
				AND g.BrandGroupCode = @paramBrandGroup AND g.BrandCode = @paramBrandCode AND g.ProcessGroup = @processGroup AND g.StatusEmp = @statusEmp
	...
	BEGIN
		...
		FROM ExeReportByGroups g
		INNER JOIN MstGenProcess p on p.ProcessGroup = g.ProcessGroup
		WHERE g.LocationCode = @paramLocationCode AND g.UnitCode = @paramUnitCode AND g.ProductionDate BETWEEN @paramProdDateFrom AND @paramProdDateTo
				AND g.BrandGroupCode = @paramBrandGroup AND g.BrandCode = @paramBrandCode AND g.Shift = @paramShift
		GROUP BY
			g.ProcessGroup,
			g.StatusEmp,
