...
BEGIN
	...
	WHILE @@FETCH_STATUS = 0 BEGIN
		...
		BEGIN
			...
			ELSE
			BEGIN
				IF((SELECT IDFlow FROM @TempTableLastTransactionLogs) = 56)
				BEGIN
					SET @status = 'APPROVED';
					SET @idNextRole = (SELECT DestinationRole FROM UtilFlows WHERE IDFlow = 56);
				END

				IF((SELECT IDFlow FROM @TempTableLastTransactionLogs) = 25)
				BEGIN
					SET @status = 'APPROVED';
					SET @idNextRole = (SELECT DestinationRole FROM UtilFlows WHERE IDFlow = 25);
				END

				IF((SELECT IDFlow FROM @TempTableLastTransactionLogs) = 26)
				BEGIN
					SET @status = 'COMPLETED';
					SET @idNextRole = 0;
				END

				IF((SELECT IDFlow FROM @TempTableLastTransactionLogs) = 24)
				BEGIN
					SET @status = 'SUBMITTED';
					SET @idNextRole = (SELECT DestinationRole FROM UtilFlows WHERE IDFlow = 24);
				END

				IF((SELECT IDFlow FROM @TempTableLastTransactionLogs) = 57)
				BEGIN
					SET @status = 'DRAFT';
					SET @idNextRole = (SELECT DestinationRole FROM UtilFlows WHERE IDFlow = 21);
				END
