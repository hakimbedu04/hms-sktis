/****** Object:  StoredProcedure [dbo].[TPOProductionEntryVerificationGenerateReport]    Script Date: 2/14/2017 1:24:08 PM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
-- Description: change hardcode PMI\User to @UserName
...
SET @TPOFeeCode = 'FEE/' + @LocationCode + '/' + @BrandGroupCode + '/' + CAST(@KPSYear AS VARCHAR) + '/' + CAST(@KPSWeek AS VARCHAR);
	...
	WHILE @StartDate <= @EndDate
		BEGIN
			...
			-- adding function for generate default value for holiday or not holiday
			--start default value
			-- update on 24/01/2017 08:38 , update dayofweek only 7 didn't generate
			IF( @DayOfWeek <> 7 )
			BEGIN
			DECLARE @IsHolidayDef INT;
			SELECT @IsHolidayDef = COUNT(*) FROM dbo.MstGenHoliday mgh WHERE mgh.LocationCode = @LocationCode AND mgh.StatusActive = '1' AND mgh.HolidayDate = @StartDate
					
					IF @IsHolidayDef <= 0
						BEGIN
							SELECT @JKNJamStdSum = mgsh.JknHour,
								   @Jl1JamSum = mgsh.Jl1Hour,
								   @JL2JamSum = mgsh.Jl2Hour,
								   @JL3JamSum   = mgsh.Jl3Hour,
								   @Jl4JamSum = mgsh.Jl4Hour
		   ...
		   ---- end update for default value
			...
			--print '1'
				...
				DECLARE @transactionCode VARCHAR(100) = 'EBL/' + @LocationCode + '/4/' + @TempBrandCode + '/' + CAST(@KPSYear AS VARCHAR) + '/' + CAST(@KPSWeek AS VARCHAR) + '/' + CAST(@DayOfWeek AS VARCHAR);
				...
				--IDFlow = 38
				IF (( @LogExist = 38 ) OR ( @LogExist = 39 ) OR ( @LogExist = 40 ))
				BEGIN
					-- REAL CALCULATION LOOP AND SUMARIZING --
					
					INSERT INTO @TempTableExists (BrandCode) VALUES (@TempBrandCode)
					...
					SELECT @Outputbox = SUM(etev.KeluarBersih)
					FROM ExeReportByProcess etev
					WHERE etev.LocationCode = @LocationCode
						AND etev.BrandCode IN (SELECT BrandCode FROM @TempTableExists)
						AND etev.KPSYear = @KPSYear
						AND etev.KPSWeek = @KPSWeek
						AND etev.ProductionDate = @StartDate
					...
					SELECT TOP 1 @ProcessGroupVerification = etev.ProcessGroup
					FROM ExeReportByProcess etev
					WHERE etev.LocationCode = @LocationCode
					...
					SET @OutputboxSum = @Outputbox;
					SET @OutputSticksSum = @OutputSticks;
			...
			SET @JKN = ROUND(@JKNJamSum, 6);
			SET @JL1 = ROUND(@JL1Sum, 6);
			SET @JL2 = ROUND(@JL2Sum, 6);
			SET @JL3 = ROUND(@JL3Sum, 6);
			SET @JL4 = ROUND(@JL4Sum, 6);
