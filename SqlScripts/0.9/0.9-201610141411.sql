/****** Object:  StoredProcedure [dbo].[BaseExeReportByProcess]    Script Date: 10/14/2016 10:35:58 AM ******/
SET ANSI_NULLS ON
GO
...
FROM
	...
	IF EXISTS (SELECT 1 FROM dbo.ExeReportByProcess WHERE LocationCode = @LocationCode AND BrandCode = @BrandCode AND ProcessGroup = @ProcessGroup AND KPSYear = @KPSYear AND KPSWeek = @KPSWeek AND ProductionDate = @ProductionDate)
		BEGIN
				DELETE FROM dbo.ExeReportByProcess WHERE LocationCode = @LocationCode AND BrandCode = @BrandCode AND ProcessGroup = @ProcessGroup and ProductionDate = @ProductionDate
		END
