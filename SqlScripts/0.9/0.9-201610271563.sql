...
ALTER PROCEDURE [dbo].[EMPJOBDATA_PopulateUpd]
AS
BEGIN
	...
	AND ((A.LAST_UPDATE_DATE >= CONVERT(DATE, @currLastUpdatedUpd) AND A.LAST_UPDATE_DATE >= A.EFFDT) 
			OR
		 (A.LAST_UPD_NO_PEN >= CONVERT(DATE, @currLastUpdatedUpd) AND A.LAST_UPD_NO_PEN >= A.EFFDT_NO_PEN)) 
	AND NOT EXISTS (SELECT * FROM MstPlantEmpUpd upd WHERE upd.EmployeeID = A.EMPLOYEE_ID)
END;