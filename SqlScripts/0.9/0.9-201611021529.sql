...
-- =============================================
-- Description: fixing @DailyJKN, @DailyJL1, @DailyJL2, @DailyJL3, @DailyJL4
-- Ticket: http://tp.voxteneo.co.id/entity/10993
-- Author: Azka
-- Version: 2.8
-- Date: 2016/11/02
-- =============================================
...
AS
    BEGIN
	   ...
	   SELECT @StartDate = StartDate,
		...
		FROM TPOFeeHdrPlan where TPOFeeCode = @TPOFeeCode
		...
		WHILE @StartDate <= @EndDate
			BEGIN
				...
				SELECT TOP 1 @Outputbox = OutputBox FROM TPOFeeProductionDailyPlan WHERE TPOFeeCode = @TPOFeeCode AND FeeDate = @StartDate
				
				SET @DailyJKN = ROUND((@Outputbox / @TotalProdBox) * @TotalProdJKN, 2);
				SET @DailyJL1 = ROUND((@Outputbox / @TotalProdBox) * @TotalProdJl1, 2);
				SET @DailyJL2 = ROUND((@Outputbox / @TotalProdBox) * @TotalProdJl2, 2);
				SET @DailyJL3 = ROUND((@Outputbox / @TotalProdBox) * @TotalProdJl3, 2);
				SET @DailyJL4 = ROUND((@Outputbox / @TotalProdBox) * @TotalProdJl4, 2);

				print @TotalProdJKN;
				print @TotalProdBox;

				-- JKN --
				IF(@TotalDibayarJKN > 0)--@TotalProdJKN > 0 AND @TotalDibayarJKN > 0 AND @totalRupiahJKN > 0)
					BEGIN
						--SET @convertJKN = (@DailyJKN / @TotalProdJKN) * @TotalDibayarJKN;
						--SET @JKNRp = (@convertJKN / @TotalDibayarJKN) * @totalRupiahJKN;
						SET @JKNRp = (@DailyJKN / @TotalDibayarJKN) * @totalRupiahJKN;
						--PRINT @TotalProdJKN;
						--PRINT @TotalDibayarJKN;
						--PRINT @COUNTS + ' '+@JKNRp;
					END
				
				-- JL1 --
				IF(@TotalDibayarJL1 > 0)--@TotalProdJl1 > 0 AND @TotalDibayarJL1 > 0 AND @totalRupiahJL1 > 0)
					BEGIN
						SET @convertJL1 = (@DailyJL1 / @TotalProdJl1) * @TotalDibayarJL1;
						SET @JL1Rp = (@DailyJL1 /@TotalDibayarJL1) * @totalRupiahJL1;
					END
				
				-- JL2 --
				IF(@TotalDibayarJL2 >0 )--@TotalProdJl2 > 0 AND @TotalDibayarJL2 > 0 AND @totalRupiahJL2 > 0)
					BEGIN
				...
				-- JL3 --
				IF(@TotalDibayarJL3 > 0)--@TotalProdJl3 > 0 AND @TotalDibayarJL3 > 0 AND @totalRupiahJL3 > 0)
					BEGIN
						SET @convertJL3 = (@DailyJL3 / @TotalProdJl3) * @TotalDibayarJL3;
						SET @JL3Rp = (@DailyJL3 /@TotalDibayarJL3) * @totalRupiahJL3;
					END
				
				-- JL4 --
				IF(@TotalDibayarJL4 >0)--@TotalProdJl4 > 0 AND @TotalDibayarJL4 > 0 AND @totalRupiahJL4 > 0)
					BEGIN
						SET @convertJL4 = (@DailyJL4 / @TotalProdJl4) * @TotalDibayarJL4;
						SET @JL4Rp = (@DailyJL4 /@TotalDibayarJL4) * @totalRupiahJL4;
					END
