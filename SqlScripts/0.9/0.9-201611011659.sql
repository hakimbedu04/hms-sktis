/****** Object:  StoredProcedure [dbo].[ExtendedExeReportByProcess]    Script Date: 11/1/2016 4:53:07 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	(
		...
		ROUND((max(e.EndingStock)/ max(psalv.StickPerPack)), 2) as EndingStock,
	...
	WHERE e.ProcessGroup = 'PACKING' AND e.LocationCode = @LocationCode AND e.UnitCode = @UnitCode AND e.BrandCode = @BrandCode AND e.ProductionDate = @ProductionDate
	...
	(
		...
		ROUND(ISNULL(((select TOP 1 EndingStock from ExeReportByProcess xy
			where 
				...
				--and KPSYear = SOURCE.KPSYear
				--and KPSWeek = SOURCE.KPSWeek
		) + (max(e.Production) / max(psalv.StickPerBox)) - (max(e.Production)/max(psalv.StickPerBox)) - max(RejectSample)),0), 2) as EndingStock,
		max(e.CreatedDate) as CreatedDate,
		max(e.CreatedBy) as CreatedBy,
		max(e.UpdatedDate) as UpdatedDate,
		max(e.UpdatedBy) as UpdatedBy
