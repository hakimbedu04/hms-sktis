/****** Object:  StoredProcedure [dbo].[BaseExeReportByProcess]    Script Date: 11/1/2016 4:53:00 PM ******/
SET ANSI_NULLS ON
GO
...
SELECT 
*,
ROUND(((BeginningStock + Production)-KeluarBersih-RejectSample), 2) AS EndingStock,
...
FROM
	(SELECT 
		...
		(CASE
			...
			WHEN SOURCE.ProcessGroup = 'WRAPPING' THEN 'StampPack' 
			ELSE 'NULL' 
		END)AS Description,
		(CASE 
			...
			WHEN SOURCE.ProcessGroup = 'WRAPPING' THEN 'Stick'
			ELSE 'NULL' 
		END)AS UOM,
		(CASE 
			...
			WHEN SOURCE.ProcessGroup = 'WRAPPING' THEN 7 
			ELSE 12 
		END)AS UOMOrder,
		ISNULL((
				CASE
				...
				(CASE 
					...
					THEN 0 
					ELSE ISNULL((select TOP 1 ROUND(EndingStock, 2) as EndingStock from ExeReportByProcess 
						where 
							ProductionDate = DATEADD(day, -1, @ProductionDate) 
							and LocationCode = @LocationCode 
							and BrandCode = @BrandCode 
	...
	WHERE UnitCode = @UnitCode and LocationCode = @LocationCode and BrandCode = @BrandCode
							and ProcessGroup = @ProcessGroup and ProductionDate = @ProductionDate
	...
	IF EXISTS (SELECT 1 FROM dbo.ExeReportByProcess WHERE LocationCode = @LocationCode AND UnitCode = @UnitCode AND BrandCode = @BrandCode AND ProcessGroup = @ProcessGroup AND KPSYear = @KPSYear AND KPSWeek = @KPSWeek AND ProductionDate = @ProductionDate)
		...
		END
			...
			(
				...
				#SOURCE.RejectSample,
				ROUND(ISNULL(#SOURCE.BeginningStock,0), 2) AS BeginningStock,
				ROUND(ISNULL(#SOURCE.EndingStock,0), 2) AS EndingStock,
				GETDATE(),
				@CreatedBy,
				GETDATE(),
				@UpdatedBy
			FROM
			  #SOURCE 
			WHERE #SOURCE.LocationCode = @LocationCode AND #SOURCE.UnitCode = @UnitCode AND #SOURCE.BrandCode = @BrandCode AND #SOURCE.ProcessGroup = @ProcessGroup AND #SOURCE.ProductionDate = @ProductionDate
