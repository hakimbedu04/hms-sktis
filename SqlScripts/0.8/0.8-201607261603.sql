/****** Object:  View [dbo].[ExePlantProductionEntryVerificationView]    Script Date: 7/25/2016 5:34:01 PM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
-- Description: Edit View for Dummy Group R Resmi and M Multiskill
    ...
    SELECT eppev.ProductionEntryCode,
		 ...
		 FROM (
			 SELECT SUM(CASE WHEN eppe.AbsentCodeEblek = 'A' THEN 1 ELSE 0 END) AS A,
				   ...
				   SUM(CASE WHEN eppe.AbsentCodeEblek = 'S' THEN 1 ELSE 0 END) AS S,
			 ...
			 FROM (
				 SELECT DISTINCT p.* FROM dbo.ExePlantProductionEntry p
				 JOIN UtilTransactionLogs trans ON trans.TransactionCode =  CASE WHEN p.StatusEmp = 'Resmi' OR p.StatusEmp = 'Multiskill' THEN STUFF(p.ProductionEntryCode, 21, 1, '') END
					UNION
				SELECT DISTINCT p.* FROM dbo.ExePlantProductionEntry p
				 JOIN UtilTransactionLogs trans ON trans.TransactionCode = p.ProductionEntryCode																
				) AS eppe
