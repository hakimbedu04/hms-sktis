/****** Object:  StoredProcedure [dbo].[INSERT_WORKER_ASSIGNMENT]    Script Date: 7/17/2016 6:12:08 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[INSERT_WORKER_ASSIGNMENT]
	...
	BEGIN TRY
		...
		BEGIN
			...
			SET @ProductionEntryCodeDestinationDummy = 'EBL' + '/' + @DestinationLocationCode 
															 + '/' + CAST(@DestinationShift as VARCHAR(1))   
															 + '/' + @DestinationUnitCode 
															 + '/' + (SUBSTRING(@DestinationGroupCode, 1, 1) + '5' + SUBSTRING(@DestinationGroupCode, 3, 2)) 
															 + '/' + @DestinationBrandCode 
															 + '/' + CAST(@Year as VARCHAR(4))    
															 + '/' + CAST(@Week as VARCHAR(2))   
															 + '/' + CONVERT(varchar,(select datepart(dw, @CurrentDate)));
			...
			BEGIN /** Assignment to PLANT Destination Location **/
					...
					BEGIN /** process group source is not DAILY **/
						INSERT INTO [dbo].[ExePlantProductionEntryVerification]
							...
							SELECT
							   ...
							   ,Flag_Manual
							FROM ExePlantProductionEntryVerification WHERE ProductionEntryCode = @ProductionEntryCodeSource
					END;
					ELSE
				...
				BEGIN /** assignment to same process**/
					...
					SELECT @prodCap = ProdCapacity, @prodActual = ProdActual, @prodTarget = ProdTarget 
