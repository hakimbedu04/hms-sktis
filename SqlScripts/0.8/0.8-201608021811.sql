...
AS
    BEGIN
		...
		SELECT TOP 1 @statusEmp = en.StatusEmp  FROM ExePlantProductionEntry en join ExePlantProductionEntryVerification ev on ev.ProductionEntryCode = en.ProductionEntryCode
		WHERE ev.LocationCode = @LocationCode
                 AND ev.KPSYear = @Year
                 AND ev.KPSWeek = @Week
                 AND ev.ProductionDate = @Date
                 AND ev.Shift = @Shift
                 AND ev.UnitCode = @Unit
                 AND ev.BrandCode = @Brand
				 AND ev.GroupCode LIKE (@dummyGroupCode + '%')
		...
		SELECT @isDummyExist = COUNT(*)
               ...
               WHERE eppev.LocationCode = @LocationCode
                 ...
                 AND eppev.UnitCode = @Unit
                 AND eppev.BrandCode = @Brand
				 AND eppev.GroupCode = @dummyGroupCode + @FlagStatusEmp;

		SET @ProductionCodeDummy = 'EBL/' + @LocationCode 
			 + '/' + CONVERT(varchar,@Shift) 
			 + '/' + @Unit 
			 + '/' + @dummyGroupCode + @FlagStatusEmp
			 + '/' + @Brand 
			 + '/' + CONVERT(varchar,@Year) 
			 + '/' + CONVERT(varchar,@Week)
			 + '/' + CONVERT(varchar,(select datepart(dw, @Date)));
		...
		BEGIN
			...
			WHERE LocationCode = @LocationCode
                 ...
                 AND UnitCode = @Unit
                 AND BrandCode = @Brand
				 AND GroupCode = @dummyGroupCode + @FlagStatusEmp;
			...
			INSERT INTO dbo.ExeReportByGroups
               ...
               SELECT eppev.LocationCode,
                      eppev.UnitCode,
                      eppev.ProcessGroup,
                      CASE WHEN LEN(eppev.GroupCode) > 4 THEN SUBSTRING(eppev.GroupCode, 0, 5) ELSE eppev.GroupCode END,
                      mgb.BrandGroupCode,
                      eppev.BrandCode,
                      CASE WHEN SUBSTRING(eppev.GroupCode, 5, 1) = 'R' THEN 'Resmi' ELSE mges.StatusEmp END,
                      eppev.ProductionDate,
                      eppev.Shift,
