...
-- Description:	Function to generate TPO Production Report
-- =============================================

CREATE FUNCTION [dbo].[GetTPOReportsProduction] 
(
...
BEGIN
	...
	BEGIN
		...
		FROM TPOFeeHdr
		WHERE LocationCode = @LocationCode
		  AND (KPSYear >= @minYear AND KPSWeek >= @minWeek)
		  AND (KPSYear <= @maxYear AND KPSWeek <= @maxWeek)
