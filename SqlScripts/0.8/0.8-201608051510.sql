/****** Object:  UserDefinedFunction [dbo].[GetReportProdStockProcessView]    Script Date: 8/5/2016 3:06:21 PM ******/
SET ANSI_NULLS ON
GO
...
-- Ticket: hot fix
-- Author: AZKA
-- Updated: 2.0 - 5/25/2016
-- =============================================

-- =============================================
-- Description: cabut rounding production, left join planning
-- Ticket: http://tp.voxteneo.co.id/entity/6821
-- Author: AZKA
-- Updated: 3.0 - 5/31/2016
-- =============================================
...
-- =============================================
-- Description: fixing variance stick and percent
-- Ticket: http://tp.voxteneo.co.id/entity/8591
-- Author: AZKA
-- Updated: 6.0 - 2016/08/05
-- =============================================
...
(
	...
	SELECT 
		...
		bp.LocationCode, 
		bp.UnitCode,
		SUM(ISNULL(bp.KeluarBersih, 0)) Production
...
SELECT 
	...
	P.Production,
	P.Production - (CASE WHEN (SELECT dbo.GetParentLocationLastChild(@LocationCode)) = 'PLANT' THEN F.TPKValue ELSE T.Planning END) as VarianceStick,
	COALESCE(((P.Production - (CASE WHEN (SELECT dbo.GetParentLocationLastChild(@LocationCode)) = 'PLANT' THEN F.TPKValue ELSE T.Planning END)) / NULLIF((CASE WHEN (SELECT dbo.GetParentLocationLastChild(@LocationCode)) = 'PLANT' THEN F.TPKValue ELSE T.Planning END), 0)) * 100, 0) as VariancePercent,
