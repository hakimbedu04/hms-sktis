/****** Object:  StoredProcedure [dbo].[TransactionLog]    Script Date: 8/28/2016 9:33:11 PM ******/
SET ANSI_NULLS ON
GO
...
AS
    BEGIN
		...
		ELSE
		BEGIN
			SELECT @RolesName = RolesName FROM UtilRoles WHERE IDRole = @IDRole;
		END
            ...
            BEGIN
			 ...
			 --SELECT @IDFunct = IDFunction FROM UtilFunctions ufun WHERE ufun.FunctionName LIKE 'ProductionCardApprovalDetail';
						...
						BEGIN
							...
							IF @IDFlow = @idflow22
								BEGIN
									...
									VALUES( @TransactionCode, @Transaction_Date, @IDFlow, '', @ActionTime, @UserName, GETDATE(), @RolesName );
								END;
				...
				BEGIN
					...
					INSERT INTO UtilTransactionLogs ( TransactionCode, TransactionDate, IDFlow, Comments, CreatedDate, CreatedBy, UpdatedDate, UpdatedBy )
					VALUES( @TransactionCode, @Transaction_Date, @IDFlow, @MessageText, @ActionTime, @UserName, GETDATE(), @RolesName );
					RETURN;
				END;
				ELSE
					BEGIN
						INSERT INTO UtilTransactionLogs ( TransactionCode, TransactionDate, IDFlow, Comments, CreatedDate, CreatedBy, UpdatedDate, UpdatedBy )
						VALUES( @TransactionCode, @Transaction_Date, 26, '', @ActionTime, @UserName, GETDATE(), @RolesName );
					END;
				RETURN;
            END;
		  ...
		  WHILE (SELECT COUNT(id) FROM @FlowLists) > 0
			BEGIN
				...
				SELECT @IDFlow = IDFlow, @MessageText = COALESCE(@MessageText, MessageText) FROM @FlowLists WHERE id = (SELECT MIN(id) FROM @FlowLists);
				 INSERT INTO UtilTransactionLogs ( TransactionCode, TransactionDate, IDFlow, Comments, CreatedDate, CreatedBy, UpdatedDate, UpdatedBy )
					VALUES( @TransactionCode, @Transaction_Date_New, @IDFlow, @MessageText, @ActionTime, @UserName, GETDATE(), @RolesName );
