/****** Object:  StoredProcedure [dbo].[GetEMSSourceData]    Script Date: 5/18/2016 12:04:37 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	Select Company,EMS.LocationCode,EMS.BrandGroupCode,EMS.BrandCode,DIS.Description,PackageQTY,ProducedQTY,UOM,ProductionDate From   (
			  ...
			  FROM [dbo].[ExeReportByGroups] where ProcessGroup = 'STAMPING' or ProcessGroup = 'WRAPPING' and ProductionDate >= @start and ProductionDate <= @enddate
	...
	Select Company,EMS.LocationCode,EMS.BrandGroupCode,EMS.BrandCode,DIS.Description,PackageQTY,ProducedQTY,UOM,ProductionDate From   (
			  ...
			  group by LocationCode,BrandCode,BrandGroupCode,ProductionDate ) AS Produced  inner join
			  (SELECT LocationCode,BrandCode,BrandGroupCode,SUM( Production ) as PackageQTY ,ProductionDate  
			  FROM [dbo].[ExeReportByGroups] where ProcessGroup = 'STAMPING' or ProcessGroup = 'WRAPPING' and ProductionDate between @start and @enddate And BrandCode = @BrandCode
			  group by LocationCode,BrandCode,BrandGroupCode,ProductionDate) AS Package on Produced.LocationCode = Package.LocationCode and Produced.BrandCode = Package.BrandCode and Produced.ProductionDate = Package.ProductionDate
			 ) ) AS EMS inner join [dbo].[MstGenBrand] AS DIS on EMS.BrandCode = DIS.BrandCode AND EMS.BrandGroupCode = DIS.BrandGroupCode inner join [dbo].[MstGenLocation] PLNT on EMS.LocationCode = PLNT.LocationCode 
	end
	...
	Select Company,EMS.LocationCode,EMS.BrandGroupCode,EMS.BrandCode,DIS.Description,PackageQTY,ProducedQTY,UOM,ProductionDate From   (
			  ...
			  group by LocationCode,BrandCode,BrandGroupCode,ProductionDate ) AS Produced  inner join
			  (SELECT LocationCode,BrandCode,BrandGroupCode,SUM( Production ) as PackageQTY ,ProductionDate  
			  FROM [dbo].[ExeReportByGroups] where ProcessGroup = 'STAMPING' or ProcessGroup = 'WRAPPING' and ProductionDate between @start and @enddate 
			  group by LocationCode,BrandCode,BrandGroupCode,ProductionDate) AS Package on Produced.LocationCode = Package.LocationCode and Produced.BrandCode = Package.BrandCode and Produced.ProductionDate = Package.ProductionDate
			 ) ) AS EMS inner join[dbo].[MstGenBrand] AS DIS on EMS.BrandCode = DIS.BrandCode AND EMS.BrandGroupCode = DIS.BrandGroupCode inner join [dbo].[MstGenLocation] PLNT on EMS.LocationCode = PLNT.LocationCode						
	...
	ELSE IF @LocationCode != 'SKT' and @BrandCode = 'All'
	 Begin
			  ...
			  group by LocationCode,BrandCode,BrandGroupCode,ProductionDate ) AS Produced  inner join
			  (SELECT LocationCode,BrandCode,BrandGroupCode,SUM( Production ) as PackageQTY ,ProductionDate  
			  FROM [dbo].[ExeReportByGroups] where ProcessGroup = 'STAMPING' or ProcessGroup = 'WRAPPING' and ProductionDate between @start and @enddate and LocationCode = @LocationCode 
			  group by LocationCode,BrandCode,BrandGroupCode,ProductionDate) AS Package on Produced.LocationCode = Package.LocationCode and Produced.BrandCode = Package.BrandCode and Produced.ProductionDate = Package.ProductionDate
			 ) ) AS EMS inner join [dbo].[MstGenBrand] AS DIS on EMS.BrandCode = DIS.BrandCode AND EMS.BrandGroupCode = DIS.BrandGroupCode inner join [dbo].[MstGenLocation] PLNT on EMS.LocationCode = PLNT.LocationCode 
	End
	Else IF @LocationCode != 'SKT' and @BrandCode != 'All'
	Begin
			  ...
			  group by LocationCode,BrandCode,BrandGroupCode,ProductionDate ) AS Produced  inner join
			  (SELECT LocationCode,BrandCode,BrandGroupCode,SUM( Production ) as PackageQTY ,ProductionDate  
			  FROM [dbo].[ExeReportByGroups] where ProcessGroup = 'STAMPING' or ProcessGroup = 'WRAPPING' and ProductionDate between @start and @enddate and LocationCode = @LocationCode And BrandCode = @BrandCode
			  group by LocationCode,BrandCode,BrandGroupCode,ProductionDate) AS Package on Produced.LocationCode = Package.LocationCode and Produced.BrandCode = Package.BrandCode and Produced.ProductionDate = Package.ProductionDate
			 ) ) AS EMS inner join [dbo].[MstGenBrand] AS DIS on EMS.BrandCode = DIS.BrandCode AND EMS.BrandGroupCode = DIS.BrandGroupCode inner join [dbo].[MstGenLocation] PLNT on EMS.LocationCode = PLNT.LocationCode 
	End
  End
