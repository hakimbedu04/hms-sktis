...
BEGIN
	...
	(
		...
		(select TOP 1 EndingStock from ExeReportByProcess sub
			where 
	...
	(
		...
		((select TOP 1 EndingStock from ExeReportByProcess xy
			where 
				...
				--and KPSYear = SOURCE.KPSYear
				--and KPSWeek = SOURCE.KPSWeek
		) + max(Production) - max(KeluarBersih) - max(RejectSample)) as EndingStock,
		max(CreatedDate) as CreatedDate,
		max(CreatedBy) as CreatedBy,
		max(UpdatedDate) as UpdatedDate,
		max(UpdatedBy) as UpdatedBy
	FROM dbo.ExeReportByProcess e WHERE e.ProcessGroup = 'STAMPING' AND e.LocationCode = @LocationCode AND e.BrandCode = @BrandCode AND e.ProductionDate = @ProductionDate
	GROUP BY e.LocationCode,e.UnitCode,e.BrandCode,e.ProductionDate,e.ProcessGroup
	...
	(
		...
		((select TOP 1 EndingStock from ExeReportByProcess xy
			where 
				...
				--and KPSYear = SOURCE.KPSYear
				--and KPSWeek = SOURCE.KPSWeek
		) + (max(e.Production) / max(psalv.StickPerBox)) - (max(e.Production)/max(psalv.StickPerBox)) - max(RejectSample)) as EndingStock,
		max(e.CreatedDate) as CreatedDate,
		max(e.CreatedBy) as CreatedBy,
		max(e.UpdatedDate) as UpdatedDate,
		max(e.UpdatedBy) as UpdatedBy
